...
with tab1:
    ...
    query = st.text_input("Buscar por SKU o nombre", key="search_products")
...
with tab2:
    ...
    if movements:
        ...
        if grouped_movements:
            ...
            fig = px.line(df_grouped, x="fecha", y="cantidad", color="tipo", title="Movimientos por día")
...
with tab3:
    st.markdown("### Reportes de Ubicaciones")
    
    status_counts = count_locations_by_status()
    col1, col2, col3 = st.columns(3)
    with col1:
        render_kpi_card("Total ubicaciones", status_counts["total"], icono="📍")
    with col2:
        render_kpi_card("Ocupadas", status_counts["ocupadas"], icono="✅", color="green")
    with col3:
        render_kpi_card("Libres", status_counts["libres"], icono="❌", color="red")
